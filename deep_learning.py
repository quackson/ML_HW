...
plot_eigvals            = True # dynamically plot maximum eigenvalues for Jacobians
...
dt  = 1.0        # time step (ms)
mem = int(10/dt) # spike memory (time steps) - used to limit PSP integration of past spikes (for performance) # TODO:
...
def kappa(x): # TODO: paper function (12)
    return (np.exp(-x/tau_L) - np.exp(-x/tau_s))/(tau_L - tau_s)

def get_kappas(n=mem):
    return np.array([kappa(i+1) for i in xrange(n)]) # 一个spike的
        ...
        if self.M == 1:
            ...
            if use_broadcast:
                if use_spiking_feedback:
                    ...
                    # 
                    for m in xrange(1, self.M-1): # TODO: 
                        if use_spiking_feedforward:
                            self.l[m].out_f(self.l[m-1].S_hist, self.l[-1].S_hist) # hist是电压，S_hist是上一层spike输出
                        else:
...
class Layer:
    ...
    def spike(self):
        ...
        self.S_hist = np.concatenate([self.S_hist[:, 1:], np.random.poisson(self.lambda_C)], axis=-1) # 是输出的柏松序列，是对lambda_C的变换，新的lambda_C变成0-1序列再加到之前的柏松spiking序列上
...
class hiddenLayer(Layer):
    ...
    def update_C(self):
        ...
        self.lambda_C = lambda_max*sigma(self.C) # 发放率，是电压的函数 # TODO:
