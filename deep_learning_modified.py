...
from utils import *
from models import Guerguiev2017BaseNode, Guerguiev2017hiddenNode, Guerguiev2017finalNode, Guerguiev2017Network
...
#         Arguments:
#             net (Guerguiev2017Network) : The network that the layer belongs to.
...
#         self.v_basal  = np.zeros((self.size, 1))
#         self.v_soma = np.zeros((self.size, 1))
#         self.spiking_rate = np.zeros((self.size, 1))
...
#         self.average_v_soma_f        = np.zeros((self.size, 1))
#         self.average_v_soma_t        = np.zeros((self.size, 1))
#         self.average_spiking_rate_f = np.zeros((self.size, 1))
#         self.average_spiking_rate_t = np.zeros((self.size, 1))
#         self.average_basal_input_f    = np.zeros((self.f_input_size, 1))
...
#         self.basal_input_hist    = np.zeros((self.f_input_size, integration_time))
#         self.v_soma_hist        = np.zeros((self.size, integration_time))
#         self.spiking_rate_hist = np.zeros((self.size, integration_time))
...
#         self.spike_hist        *= 0
#         self.basal_input_hist    *= 0
#         self.v_soma_hist        *= 0
#         self.spiking_rate_hist *= 0

#         self.E       *= 0
#         self.delta_W *= 0
#         self.delta_b *= 0

#         self.average_v_soma_f        *= 0
#         self.average_v_soma_t        *= 0
#         self.average_spiking_rate_f *= 0
#         self.average_spiking_rate_t *= 0
#         self.average_basal_input_f    *= 0
...
#             self.basal_input = np.dot(f_input, kappas)
#         else:
#             self.basal_input = f_input

#         self.basal_input_hist[:, self.integration_counter % integration_time] = self.basal_input[:, 0]

#         self.v_basal = np.dot(self.net.W[self.m], self.basal_input) + self.net.b[self.m]
...
#         if phase == "forward":
#             self.average_v_soma_f        = np.mean(self.v_soma_hist, axis=-1)[:, np.newaxis]
#             self.average_spiking_rate_f = np.mean(self.spiking_rate_hist, axis=-1)[:, np.newaxis]
#             self.average_basal_input_f    = np.mean(self.basal_input_hist, axis=-1)[:, np.newaxis]
#         elif phase == "target":
#             self.average_v_soma_t        = np.mean(self.v_soma_hist, axis=-1)[:, np.newaxis]
#             self.average_spiking_rate_t = np.mean(self.spiking_rate_hist, axis=-1)[:, np.newaxis]
...
#         out_spike = out_spike.numpy()
#         #print(out_spike.size)
#         self.spike_hist = np.concatenate([self.spike_hist[:, 1:], out_spike], axis=-1)
...
#         super().__init__(net, m, f_input_size)

#         self.b_input_size = b_input_size

#         self.v_apical             = np.zeros((self.size, 1))
#         self.delta_Y = np.zeros(self.net.Y[self.m].shape)

#         self.average_v_apical_f        = np.zeros((self.size, 1))
#         self.average_v_apical_t        = np.zeros((self.size, 1))
#         if update_feedback_weights:
#             self.average_apical_input_f = np.zeros((self.b_input_size, 1))
        
#         self.alpha_f            = np.zeros((self.size, 1))
#         self.alpha_t            = np.zeros((self.size, 1))

#         self.v_apical_hist        = np.zeros((self.size, integration_time))
#         self.apical_input_hist    = np.zeros((self.b_input_size, integration_time))
...
#         super().clear_vars()
#         self.v_apical             *= 0
#         self.v_apical_hist        *= 0
#         self.apical_input_hist    *= 0
#         self.delta_Y *= 0

#         self.average_v_apical_f        *= 0
#         self.average_v_apical_t        *= 0
#         if update_feedback_weights:
#             self.average_apical_input_f *= 0
...
#         if not use_backprop:
#             self.E = (self.alpha_t - self.alpha_f)*-k_B*lambda_max*deriv_sigma(self.average_v_soma_f)

#             if record_backprop_angle and not use_backprop and calc_E_bp:
#                 self.E_bp = (np.dot(self.net.W[self.m+1].T, self.net.l[self.m+1].E_bp)*k_B*lambda_max*deriv_sigma(self.average_v_soma_f))
#         else:
#             self.E_bp = (np.dot(self.net.W[self.m+1].T, self.net.l[self.m+1].E_bp)*k_B*lambda_max*deriv_sigma(self.average_v_soma_f))
#             self.E    = self.E_bp
...
#         self.delta_W        = np.dot(self.E, self.average_basal_input_f.T)
...
#         E_inv = (lambda_max*sigma(self.average_v_soma_f) - self.alpha_f)*-deriv_sigma(self.average_v_apical_f)

#         self.delta_Y        = np.dot(E_inv, self.average_apical_input_f.T)
...
#             self.apical_input = np.dot(b_input, kappas)
#         else:
#             self.apical_input = b_input

#         self.apical_input_hist[:, self.integration_counter % integration_time] = self.apical_input[:, 0]

#         if use_feedback_bias:
#             self.v_apical = np.dot(self.net.Y[self.m], self.apical_input) + self.net.c[self.m]
#         else:
#             self.v_apical = np.dot(self.net.Y[self.m], self.apical_input)
#         self.v_apical_hist[:, self.integration_counter % integration_time] = self.v_apical[:, 0]
...
#                 self.v_soma_dot = -g_l*self.v_soma + g_b*(self.v_basal - self.v_soma) + g_a*(self.v_apical - self.v_soma)
#             else:
...
#         self.v_soma_hist[:, self.integration_counter % integration_time] = self.v_soma[:, 0]

#         self.spiking_rate = lambda_max*sigma(self.v_soma)
#         self.spiking_rate_hist[:, self.integration_counter % integration_time] = self.spiking_rate[:, 0]
...
#         self.average_v_apical_f[plateau_indices] = np.mean(self.v_apical_hist[plateau_indices], axis=-1)[:, np.newaxis]
...
#         # calculate average apical potentials for neurons undergoing plateau potentials
#         self.average_v_apical_t[plateau_indices] = np.mean(self.v_apical_hist[plateau_indices], axis=-1)[:, np.newaxis]
...
#         if phase == "forward":
#             if update_feedback_weights:
#                 self.average_apical_input_f = np.mean(self.apical_input_hist, axis=-1)[:, np.newaxis]
#         elif phase == "target":
#             if update_feedback_weights:
#                 self.average_apical_input_t = np.mean(self.apical_input_hist, axis=-1)[:, np.newaxis]
...
#         self.E = (self.average_spiking_rate_t - lambda_max*sigma(self.average_v_soma_f))*-k_D*lambda_max*deriv_sigma(self.average_v_soma_f)

#         if use_backprop or (record_backprop_angle and calc_E_bp):
#             self.E_bp = (self.average_spiking_rate_t - lambda_max*sigma(self.average_v_soma_f))*-k_D*lambda_max*deriv_sigma(self.average_v_soma_f)
...
#         if use_conductances:
#             if phase == "forward":
#                 self.v_soma_dot = -g_l*self.v_soma + g_d*(self.v_basal - self.v_soma)
#             elif phase == "target":
#                 self.v_soma_dot = -g_l*self.v_soma + g_d*(self.v_basal - self.v_soma) + self.I
#             self.v_soma += self.v_soma_dot*dt
#         else:
#             if phase == "forward":
#                 self.v_soma = k_D*self.v_basal
#             elif phase == "target":
#                 self.v_soma = self.k_D2*self.v_basal + self.k_E*E_E + self.k_I*E_I
...
def load_simulation(latest_epoch, folder_name, simulations_folder=default_simulations_folder):
    '''
        ...
        Returns:
            net (Guerguiev2017Network)             : Guerguiev2017Network object with re-loaded weights.
...
#             root_dir = 'MNIST'
#             print(root_dir)
#             #train_set = NMNIST(root_dir, train=True, data_type='event')
#             #print("load data to np")
#             #NMNIST.read_bin_save_to_np('train-images.idx3-ubyte','x_train.npy')
#             #NMNIST.read_bin_save_to_np('t10k-images.idx3-ubyte','x_test.npy')
#             #NMNIST.read_bin_save_to_np('train-labels.idx1-ubyte','t_train.npy')
#             #NMNIST.read_bin_save_to_np('t10k-labels.idx1-ubyte','t_test.npy')
...
#         try:
#             '''
#             print('MNIST downloadable', NMNIST.downloadable())
#             root_dir = 'MNIST'
#             train_set = NMNIST(root_dir, train=True, data_type='event')
#             '''
...
#         # create all layers
#         if self.M == 1:
#             self.l.append(Guerguiev2017finalNode(net=self, m=-1, f_input_size=self.n_in))
#         else:
#             if use_broadcast:
#                 self.l.append(Guerguiev2017hiddenNode(net=self, m=0, f_input_size=self.n_in, b_input_size=self.n[-1]))
#                 for m in xrange(1, self.M-1):
#                     self.l.append(Guerguiev2017hiddenNode(net=self, m=m, f_input_size=self.n[m-1], b_input_size=self.n[-1]))
#             else:
#                 self.l.append(Guerguiev2017hiddenNode(net=self, m=0, f_input_size=self.n_in, b_input_size=self.n[1]))
#                 for m in xrange(1, self.M-1):
#                     self.l.append(Guerguiev2017hiddenNode(net=self, m=m, f_input_size=self.n[m-1], b_input_size=self.n[m+1]))
#             self.l.append(Guerguiev2017finalNode(net=self, m=self.M-1, f_input_size=self.n[-2]))
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_f(self.l[m-1].spike_hist, self.l[-1].spike_hist)
#                         else:
#                             self.l[m].out_f(self.l[m-1].spiking_rate, self.l[-1].spike_hist)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_f(self.l[m-1].spike_hist, self.l[-1].spiking_rate)
#                         else:
#                             self.l[m].out_f(self.l[m-1].spiking_rate, self.l[-1].spiking_rate)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_f(self.l[m-1].spike_hist, self.l[m+1].spike_hist)
#                         else:
#                             self.l[m].out_f(self.l[m-1].spiking_rate, self.l[m+1].spike_hist)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_f(self.l[m-1].spike_hist, self.l[m+1].spiking_rate)
#                         else:
#                             self.l[m].out_f(self.l[m-1].spiking_rate, self.l[m+1].spiking_rate)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_t(self.l[m-1].spike_hist, self.l[-1].spike_hist)
#                         else:
#                             self.l[m].out_t(self.l[m-1].spiking_rate, self.l[-1].spike_hist)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_t(self.l[m-1].spike_hist, self.l[-1].spiking_rate)
#                         else:
#                             self.l[m].out_t(self.l[m-1].spiking_rate, self.l[-1].spiking_rate)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_t(self.l[m-1].spike_hist, self.l[m+1].spike_hist)
#                         else:
#                             self.l[m].out_t(self.l[m-1].spiking_rate, self.l[m+1].spike_hist)
...
#                     for m in xrange(1, self.M-1):
#                         if use_spiking_feedforward:
#                             self.l[m].out_t(self.l[m-1].spike_hist, self.l[m+1].spiking_rate)
#                         else:
#                             self.l[m].out_t(self.l[m-1].spiking_rate, self.l[m+1].spiking_rate)
...
#             self.v_apical_hists = [ np.zeros((l_f_phase, self.l[m].size)) for m in xrange(self.M-1) ]
#             self.v_basal_hists = [ np.zeros((l_f_phase, self.l[m].size)) for m in xrange(self.M) ]
#             self.v_soma_hists = [ np.zeros((l_f_phase, self.l[m].size)) for m in xrange(self.M) ]
...
#             self.J_betas.append(np.multiply(lambda_max*deriv_sigma(self.l[-1].average_v_soma_f), k_D*self.W[-1]))
#             self.J_gammas.append(np.multiply(deriv_sigma(np.dot(self.Y[-2], lambda_max*sigma(self.l[-1].average_v_soma_f))), self.Y[-2]))
...
#         if record_voltages:
#             # initialize voltage arrays
#             self.v_apical_hists = [ np.zeros((l_t_phase, self.l[m].size)) for m in xrange(self.M-1)]
#             self.v_basal_hists = [ np.zeros((l_t_phase, self.l[m].size)) for m in xrange(self.M)]
#             self.v_soma_hists = [ np.zeros((l_t_phase, self.l[m].size)) for m in xrange(self.M)]
...
#                     if m != self.M-1:
#                         self.v_apical_hists[m][time, :] = self.l[m].v_apical[:, 0]
#                     self.v_basal_hists[m][time, :] = self.l[m].v_basal[:, 0]
#                     self.v_soma_hists[m][time, :] = self.l[m].v_soma[:, 0]
...
#         if record_loss:
#             self.loss = ((self.l[-1].average_spiking_rate_t - lambda_max*sigma(self.l[-1].average_v_soma_f)) ** 2).mean()

#         for m in xrange(self.M-1, -1, -1):
#             # reset averages
#             self.l[m].average_v_soma_f     *= 0
#             self.l[m].average_v_soma_t     *= 0
#             self.l[m].average_basal_input_f *= 0

#             if m == self.M-1:
#                 self.l[m].average_spiking_rate_f *= 0
#                 self.l[m].average_spiking_rate_t *= 0
#             else:
#                 self.l[m].average_v_apical_f     *= 0
#                 self.l[m].average_v_apical_t     *= 0
#                 self.l[m].average_spiking_rate_f *= 0
#                 if update_feedback_weights:
...
#                 if record_training_error:
#                     sel_num = np.argmax(np.mean(self.l[-1].average_v_soma_f.reshape(-1, self.n_neurons_per_category), axis=-1))
